...
public abstract class AbstractWBufferStrategyBenchmarkTest {
    ...
    protected AbstractWBufferStrategyBenchmarkTest(BufferStrategy strategy, BufferTestType testType) {
    ...
    public void testStrategy() throws IOException {
        ...
        // warm-up loop
        for (int i = 0; i < 2; i++) {
            BenchmarkTestUtils.createAndRunPlan(
                    "UPDATE table1 SET t1_intfield1 = t1_intfield1 + 1 " +
                            "WHERE t1_intfield2 - (t1_intfield2 / 2) * 2 = 0;", testData.db());
            BenchmarkTestUtils.createAndRunPlan("UPDATE table2 SET t2_stringfield1 = 'a" + i + "';", testData.db());
        ...
        for (int i = 0; i < NUM_REPEATS; i++) {
            ...
            times[i] = 0;
            for (int j = 0; j < 5; j++) {
                times[i] += BenchmarkTestUtils.createAndRunPlan(
                        "UPDATE table1 SET t1_intfield1 = t1_intfield1 + 1 " +
                                "WHERE t1_intfield2 - (t1_intfield2 / 2) * 2 = 0;", testData.db());
                times[i] += BenchmarkTestUtils.createAndRunPlan(
                        "UPDATE table2 SET t2_stringfield1 = 'a" + j * i +"';", testData.db());
