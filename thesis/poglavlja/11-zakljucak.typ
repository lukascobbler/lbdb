...
Iako je _LBDB_ SUBP funkcionalan i pruža zadovoljavajuću količinu _SQL_ jezika, za neke delove implementacije se može kazati da im fali još dosta rada. Odlučio sam da napravim presek kod ovih stvari, da bi postigao funkcionalnu implementaciju u doglednom vremenskom periodu, makar ona ne bila savršena.
...
- čuvanje vrednosti različitih dužina u okviru iste kolone (_String_, to jest _*VAR*__CHAR_ tip) je nemoguće, jer je uvek potrebno znati unapred veličinu svih vrednosti. Da bi se mogle smestiti sve vrednosti do te dužine, finalna veličina kolone će uvek biti maksimalna. Na primer za _VARCHAR(255)_ tip i `"ab"` vrednost će se zauzeti $255 * 3$ bajtova umesto $2 * 3$ bajtova. Množenje sa $3$ se radi zato što se _String_-ovi kodiraju sa _UTF-8_ kodiranjem, a on zauzima tri bajta za svaki karakter u okviru mog _Java_ okruženja,
...
Kao što je već diskutovano u poglavlju o planiranju i planerima, planer _LBDB_ sistema ne koristi skoro nijednu naprednu tehniku planiranja. Brzina izvršavanja upita dosta zavisi od redosleda tabela u upitu, uslov filtriranja se primenjuje nakon ulančavanja svih tabela umesto izolovano po tabeli, selektivnost i procene broja _NULL_ i jedinstvenih vrednosti se ne koriste, ...
