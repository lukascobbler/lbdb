...
- čuvanje vrednosti različitih dužina u okviru iste kolone (_String_, to jest _*VAR*__CHAR_ tip) je nemoguće, jer je uvek potrebno znati unapred veličinu svih vrednosti. Da bi se mogle smestiti sve vrednosti do te dužine, finalna veličina kolone će uvek biti maksimalna. Na primer za _VARCHAR(255)_ tip i `"ab"` vrednost će se zauzeti $255 * 3$ bajtova umesto $2 * 3$ bajtova. Množenje sa $3$ se radi zato što se _String_-ovi kodiraju sa _UTF-8_ kodingom, a on zauzima tri bajta za svaki karakter u okviru mog Java okruženja,
